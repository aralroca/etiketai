 ...
 */
function coords(box, { w, h }, canvasSize) {
  const { fixW, fixH } = fixers({ w, h }, canvasSize)
  const [b1, b2, b3, b4] = box.map((b, i) => i === 0 || i === 2 ? fixW(b) : fixH(b))
...
function txt(boxes, labelsIndx, labels, imgRes, canvasSize) {
  return boxes
    .map((box, index) => `${labelsIndx[labels[index]]} ${coords(box, imgRes, canvasSize)}`)
    .join('\n')
}

export default async function getYoloLabels(allBoxes, files, labels, size) {
  ...
  ...allBoxes.map((boxes, i) => ({
    dataurl: `data:text/txt,${txt(boxes, labelsIndx, labels[i], resolutions[i], size)}`,
